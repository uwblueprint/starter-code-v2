...
The easiest way to test CRUD is to use either the REST or GraphQL entities endpoints. Suggested tools include Postman, Insomnia, or cURL for REST, and the GraphQL playground (http://localhost:5001/graphql) or Altair for GraphQL. You can also choose to test through the frontend, which calls a large subset of these endpoints.
